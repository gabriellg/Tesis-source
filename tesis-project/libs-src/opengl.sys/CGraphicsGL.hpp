...
typedef void (*CGraphicsGL_FPtr_initFont)(
                        unsigned long fontListBase,
                        unsigned long *fontHeightPixel);

class CGraphicsGL: public IGraphics
{
    public:
        ...
        void initFont3D(CGraphicsGL_FPtr_initFont func_initFont);
        void removeBlocksRegisters(void);
        ...
        virtual bool hasManagerBlocks(void) const { return true; };

        virtual unsigned long beginBlock(void);

        virtual void endBlock(unsigned long idBlock);

        virtual void drawBlock(unsigned long idBlock);
        ...
        void setModeTransparency(void);
        void setModeOpaque(void);

    private:

        struct prv_dataPrivateGraphicsGL_t *m_dataPrivate;
};
